  ...
  return (
    <main className="min-h-screen bg-gray-50/50 pb-20">
      <header className="bg-gradient-to-r from-slate-800 to-slate-900 text-white p-6 md:px-12 shadow-lg flex justify-between items-center sticky top-0 z-40">
        ...
        <Link
          href="/admin"
          className="bg-white/10 hover:bg-white/20 px-5 py-2.5 rounded-full text-sm font-bold transition-all flex items-center gap-2"
        >
          ...
          <form action={adicionarCategoria} className="space-y-4">
            <div>
              ...
              <input
                ...
                name="nome"
                required
                className="w-full bg-gray-50 border border-gray-200 rounded-2xl px-5 py-3.5 outline-none focus:ring-2 focus:ring-primary"
            ...
            <button
              type="submit"
              className="w-full bg-slate-900 hover:bg-slate-800 text-white font-black py-4 rounded-2xl transition-all shadow-xl active:scale-[0.98]"
            >
              Criar Categoria
            </button>
          </form>
          ...
          <div className="flex flex-col gap-3">
            {categoriasDoBanco.length === 0 ? (
              ...
              categoriasDoBanco.map((cat) => (
                <div
                  key={cat.id}
                  className="flex items-center justify-between p-4 bg-gray-50 border border-gray-200 rounded-2xl"
                >
